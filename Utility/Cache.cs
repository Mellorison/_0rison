...
namespace _0rison {
    ...
    class Fonts {
        ...
        public static SFML.Graphics.Font DefaultFont {
            get {
                if (defaultFont == null) {
                    defaultFont = Load(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("0rison.CONSOLA.TTF"));
                }
                return defaultFont;
            }
        }
        ...
        internal static SFML.Graphics.Font Load(string path) {
            ...
            if (Files.IsUsingDataPack(path)) {
                ...
                if (File.Exists(path)) {
                    fonts.Add(path, new SFML.Graphics.Font(path)); // Can't load font with bytes from path?
                }
                else { // This should work because we already checked FileExists above
                    fonts.Add(path, new SFML.Graphics.Font(Files.AssetsFolderPrefix + path)); // Can't load font with bytes from path?
                }
            }
            return fonts[path];
        }

        public static SFML.Graphics.Font Load(Stream stream) {
