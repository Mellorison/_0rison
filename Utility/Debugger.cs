...
namespace _0rison {
    ...
    public class Debugger {
        ...
        [_0risonCommand(alias: "help", helpText: "Shows help.")]
        void CmdHelp() {
            Log("", false);
            ...
            var maxGroupNameLength = cmds.Max(c => Get_0risonCommand(c.Key).Group);
            
            Log("== Available Commands:", false);
            cmds // this looks so stupid 
                //.OrderBy(kv => kv.Key)
                .GroupBy(kv => Get_0risonCommand(kv.Key).Group)
                .OrderBy(kv => kv.Key)
                .Each(group => {
                    if (group.Key != "") {
                        Log("", false);
                    ...
                    group.Each(cmd => {
                        var attr = Get_0risonCommand(cmd.Value);
                        var s = cmd.Key;
                        if (attr.HelpText != "") {
        ...
        _0risonCommand Get_0risonCommand(MethodInfo mi) {
            return (_0risonCommand)mi.GetCustomAttributes(typeof(_0risonCommand), false)[0];
        }

        _0risonCommand Get_0risonCommand(string commandName) {
            return Get_0risonCommand(commands[commandName]);
        }

        Dictionary<string, MethodInfo> GetActiveCommands() {
            return commands.Where(c => {
                var attr = Get_0risonCommand(c.Key);
                return enabledGroups.Contains(attr.Group) || Get_0risonCommand(c.Key).Group == "";
        ...
        void SendCommand(string str) {
            ...
            if (GetActiveCommands().ContainsKey(cmdName)) {
                ...
                var attr = Get_0risonCommand(cmdName);
                if (!attr.IsBuffered) {
                    ExecuteCommand();
                }
            }
            ...
            if (commands.ContainsKey(methodName)) {
                if (usageMode) {
                    ...
                    try {
                        ...
                        if (Get_0risonCommand(methodName).IsBuffered) {
                            executionError = true;
                        }
                        else {
                            ErrorFlash();
        ...
        void ShowUsage(string methodName) {
            ...
            var usageStr = Get_0risonCommand(methodName).UsageText;
            if (usageStr != "") {
                Log("", false);
                Log(usageStr, false);
            }
            ...
            AppDomain.CurrentDomain.GetAssemblies()
                .Each(a => a.GetTypes()
                    .Each(t => t.GetMethods(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static)
                        .Each(m => {
                            if (m.IsDefined(typeof(_0risonCommand), false)) {
                                var key = m.Name.ToLower();
                                var attr = Get_0risonCommand(m);
                                if (attr.Alias != "") {
        ...
        internal Debugger(Game game) {
            ...
            img_0rison = new Image(new Texture(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("0rison.ORISON LOGO(1).png.")));

            img_0rison.Batchable = false;

            img_0rison.CenterOrigin();
            img_0rison.Scroll = 0;
            ...
            Log("== 0rison Console Initialized!");
        ...
        internal void UpdateSurface() {
            ...
            img_0rison.X = renderSurface.HalfWidth;
            img_0rison.Y = renderSurface.HalfHeight;
        ...
        internal void Update() {
            ...
            img_0rison.Alpha = imgOverlay.Alpha * 0.25f;
    ...
    public class _0risonCommand : Attribute {
        /// <summary>
        ...
        public _0risonCommand(string alias = "", string usageText = "", string helpText = "", string group = "", bool isBuffered = false) {
            Alias = alias;
            UsageText = usageText;
            HelpText = helpText;
            Group = group;
